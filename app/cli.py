			...
			for file in files:
				if (not file.startswith('.')):
					...
					with open(full_path, 'rb') as data:
						click.echo(destination_folder + full_path[len(path)+0:])
						bucket.put_object(Key=folder_prefix + destination_folder + full_path[len(path)+0:], Body=data)
	else:
...
@with_appcontext
@click.option('--env', '-e', type=click.Choice(['dev', 'prod', 'none']), default='none')
...
# flask s3uploadfile --folder --env prod --destination destination/foler/on/s3/  path/to/folder
